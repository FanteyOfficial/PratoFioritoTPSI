...
class Minesweeper:
    ...
    def __init__(self, tk, client, name):
        self.client = client
        self.name = name
    ...
    def setup(self):
        ...
        self.clickedCount = 0
        self.startTime = datetime.now()  # Update startTime here
    ...
    def onClick(self, tile):
        ...
        if tile["state"] != STATE_CLICKED:
            ...
            self.clickedCount += 1
            # Update score
            score = calculate_score(self.startTime, datetime.now(), self.flagCount)
            send_score(score, self.client, self.name)  # Send updated score to server
...
def send_score(score, client, client_name):
    client.sendall(f"{score}:{client_name}".encode())

def calculate_score(start_time, end_time, flag_count):
    # Calculate time taken in seconds
    time_taken = (end_time - start_time).total_seconds()

    # You can adjust this formula based on your game's rules
    score = 1000 - time_taken - flag_count * 10
...
def main():
    name = f"Client id: {random.randint(0, 2000)}"
    ...
    minesweeper = Minesweeper(window, client, name)
    # Run event loop
    window.mainloop()
